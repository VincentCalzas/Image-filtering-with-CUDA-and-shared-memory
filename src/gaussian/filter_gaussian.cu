...
// Part 4a: Separable Gaussian Filter
// ============================================================================
//
// Weighted convolution filter using a Gaussian approximation instead of
// uniform box weights. Uses separable 1x9 / 9x1 passes with weights:
//   [1, 2, 4, 8, 16, 8, 4, 2, 1]  (sum = 46)
//
// Visual improvement over the box filter:
//   - Smoother transitions (no visible blockiness)
//   - Natural-looking blur that preserves more detail
//
// The Gaussian kernel is stored in __constant__ memory for fast, cached,
// broadcast access by all threads. Falls back to box filter for non-9x9 sizes.
//
// Per the Central Limit Theorem, repeated box filter convolutions converge
// toward a Gaussian — this kernel approximates that in a single step.
//
// Compile:
//   cd src/gaussian && nvcc filter_gaussian.cu -o ../../filter_gaussian \
...
// Gaussian kernel weights for 1×9 filter
// Weights: [1, 2, 4, 8, 16, 8, 4, 2, 1] (sum = 46)
__constant__ int gaussianWeights[9] = {1, 2, 4, 8, 16, 8, 4, 2, 1};
__constant__ int gaussianSum = 46;

// Horizontal filter (1×N) - processes rows with Gaussian weights
                                  ...
                                  const int kernelsizex)
{
	...
	// Apply horizontal filter with Gaussian weights
	if (x < imagesizex && y < imagesizey)
	{
		unsigned int sumx = 0, sumy = 0, sumz = 0;
		
		// For 1×9 Gaussian filter (kernelsizex = 4)
		if (kernelsizex == 4)
		{
			// Convolve with Gaussian weights [1, 2, 4, 8, 16, 8, 4, 2, 1]
			for(int dx = -kernelsizex; dx <= kernelsizex; dx++)
			{
				int sharedX = localX + dx;
				int weight = gaussianWeights[dx + kernelsizex]; // Center at index 4
				
				sumx += weight * sharedMem[localY][sharedX][0];
				sumy += weight * sharedMem[localY][sharedX][1];
				sumz += weight * sharedMem[localY][sharedX][2];
			}
			
			out[(y*imagesizex+x)*3+0] = sumx / gaussianSum;
			out[(y*imagesizex+x)*3+1] = sumy / gaussianSum;
			out[(y*imagesizex+x)*3+2] = sumz / gaussianSum;
		}
		else
		{
...
// Vertical filter (N×1) - processes columns with Gaussian weights
                                ...
                                const int kernelsizey)
{
	...
	// Apply vertical filter with Gaussian weights
	if (x < imagesizex && y < imagesizey)
	{
		unsigned int sumx = 0, sumy = 0, sumz = 0;
		
		// For 9×1 Gaussian filter (kernelsizey = 4)
		if (kernelsizey == 4)
		{
			// Convolve with Gaussian weights [1, 2, 4, 8, 16, 8, 4, 2, 1]
			for(int dy = -kernelsizey; dy <= kernelsizey; dy++)
			{
				...
				int weight = gaussianWeights[dy + kernelsizey]; // Center at index 4
				
				sumx += weight * sharedMem[sharedY][localX][0];
				sumy += weight * sharedMem[sharedY][localX][1];
				sumz += weight * sharedMem[sharedY][localX][2];
			}
...
void computeImages(int kernelsize)
{
	...
	const char* filterType = (kernelsize == 4) ? "Gaussian [1,2,4,8,16,8,4,2,1]" : "Box";
	printf("\n=== Separable %s Filter: %dx%d = (%dx1) * (1x%d) ===\n", 
	       filterType, filterSize, filterSize, filterSize, filterSize);
...
int main( int argc, char** argv) 
{
	...
	printf("\n\n");
	printf("╔════════════════════════════════════════════════╗\n");
	printf("║  CUDA Filter - GAUSSIAN SEPARABLE Version     ║\n");
	printf("║  Weights: [1, 2, 4, 8, 16, 8, 4, 2, 1]        ║\n");
	printf("╚════════════════════════════════════════════════╝\n");
	...
	printf(">>> Weights: [1, 2, 4, 8, 16, 8, 4, 2, 1] / 46 <<<\n");
	computeImages(4);
